...
import footballPlayer from '../dummy.json';
...
const handlers = [
    ...
    rest.get("/footballPlayer", async(req, res, ctx) => {
    ...
    // post 메서드. get과 마찬가지
    rest.post("/footballPlayer", async(req, res, ctx) => {
        ...
        return res(ctx.status(201), ctx.json(footballPlayer));
    }),
];
